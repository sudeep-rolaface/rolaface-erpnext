...
# NOTE: Removed conflicting "from frappe.utils.file_manager import save_file" import.
# The custom save_file_to_frappe() below replaces it entirely.
...
# ─────────────────────────────────────────────────────────────────────────────
# FIX 1: save_file import removed (it conflicted with this custom function).
#         Using save_file_to_frappe() everywhere instead.
#         frappe.get_site_path() resolves the correct path on any server.
#         Files saved to public/files/ — Nginx serves them at /files/<name>.
# ─────────────────────────────────────────────────────────────────────────────
def save_file_to_frappe(file_input):
    ...
    Supports: file object (multipart), base64 string, or external URL.
    """
    try:
        site_path = frappe.get_site_path("public", "files")
        os.makedirs(site_path, exist_ok=True)

        # Case 1: File object from multipart/form-data
...
def ensure_account_and_cost_center(
    ...
    Creates them if missing and links parent accounts correctly.
    """
    messages = []
    ...
    try:
        ...
        return {"status": "success", "status_code": 200, "message": "; ".join(messages)}
    except Exception as e:
        return {"status": "fail", "status_code": 500, "message": f"Error creating accounts or cost centers: {str(e)}"}
...
def validate_parent_company(parent_company):
    if frappe.db.exists("Company", parent_company):
        ...
        send_response(status="fail", message=f"Parent Company '{parent_company}' not found.", status_code=400, http_status=400)
        return False
...
def get_companies_api():
    try:
        ...
        if total_companies == 0:
            return send_response(status="success", message="No companies found.", data=[], status_code=200, http_status=200)
        ...
        return send_response(status="fail", message="Failed to fetch companies", data={"error": str(e)}, status_code=500, http_status=500)
...
def get_company_api():
    try:
        ...
        if not custom_company_id:
            return send_response(status="fail", message="Company id is required (custom_company_id)", status_code=400, http_status=400)

        # ─────────────────────────────────────────────────────────────────────
        # FIX 2: 'default' is a MySQL reserved keyword.
        #   frappe.get_all() builds "SELECT default FROM ..." which MySQL rejects.
        #   Solution: use raw SQL with backtick-escaped `default` column.
        # ─────────────────────────────────────────────────────────────────────
        bank_accounts_docs = frappe.db.sql(
            """
            SELECT id, accountno, accountholdername, bankname, swiftcode,
                   sortcode, branchaddress, currency, dateadded,
                   `default`, openingbalance
            FROM `tabCompany Accounts`
            WHERE company_id = %s
            """,
            custom_company_id,
            as_dict=True
        ...
        return send_response(status="success", message="Company retrieved successfully", data=data, status_code=200, http_status=200)
        ...
        return send_response(status="fail", message=str(e), status_code=500, http_status=500)
...
def delete_company_api():
    try:
        ...
        if not custom_company_id:
            return send_response(status="fail", message="Custom Company ID is required for delete", status_code=400, http_status=400)
        ...
        return send_response(status="success", message="Company and related data deleted successfully", status_code=200, http_status=200)
...
def update_company_info():
    try:
        ...
        if not custom_company_id:
            send_response(status="fail", message="custom_company_id is required", status_code=400, http_status=400)
            return

        company = frappe.get_doc("Company", {"custom_company_id": custom_company_id})
        if not company:
            send_response(status="fail", message="Company not found", status_code=404, http_status=404)
            return
        ...
        company.save()
        frappe.db.commit()
        send_response(status="success", message="Company information updated successfully", status_code=200, http_status=200)
        return

    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "update_company_info API Error")
        send_response(status="fail", message=str(e), status_code=500, http_status=500)
        return
...
def create_company_api():
    ...
    if not isinstance(accounts_map, list):
        return send_response(status="fail", message="bankAccounts should be a list.", status_code=400, http_status=400)

    # ─────────────────────────────────────────────────────────────────────────
    # FIX 3: default_count logic was broken — it was recalculating from the
    #   already-processed list at the START of each loop iteration (before the
    #   current bank was appended), then also doing default_count += is_default
    #   at the end, causing double-counting and wrong validation timing.
    #   Fix: count is_default for each bank and check AFTER incrementing.
    # ─────────────────────────────────────────────────────────────────────────
    bank_accounts = []
    default_count = 0
    for bank in accounts_map:
        ...
        # Check AFTER incrementing so the current bank is counted
        if default_count > 1:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # FIX 4: Was using `currency` (a loop variable from the bank loop) instead
    #   of `baseCurrency`. This caused the wrong currency to be set on the
    #   company, and the validation "if not currency" checked the wrong value.
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # FIX 4 continued: validate baseCurrency, not the loop variable `currency`
    if not baseCurrency:
        return send_response(status="fail", message="Base currency is required", status_code=400, http_status=400)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # FIX 4 continued: use baseCurrency (not `currency`) for default_currency
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # FIX 5: Company must be inserted BEFORE related docs are saved.
    #   Previously all selling/buying terms were saved before company.insert(),
    #   so they referenced a company ID (next_id) that didn't exist in DB yet,
    #   risking FK violations and orphaned records on failure.
    #   Fix: insert company first, then save all related docs.
    # ─────────────────────────────────────────────────────────────────────────
...
def update_company_api():
    ...
    def send_fail(msg, status_code=400, http_status=400):
        return send_response(status="fail", message=msg, status_code=status_code, http_status=http_status)
    ...
    for account in bank_accounts:
        ...
        if account_id and account_id in existing_ids:
            ...
            acc_doc = frappe.new_doc("Company Accounts")
            acc_doc.company_id = custom_company_id
            acc_doc.id = "{:08d}".format(random.randint(0, 99999999))
        ...
        if is_default:
            # ─────────────────────────────────────────────────────────────────
            # FIX 2 (update path): 'default' is MySQL reserved keyword.
            #   frappe.get_all() with filters={"default": 1} builds broken SQL.
            #   Solution: raw SQL with backtick-escaped `default` column.
            # ─────────────────────────────────────────────────────────────────
            other_defaults = frappe.db.sql(
                """
                SELECT name FROM `tabCompany Accounts`
                WHERE company_id = %s AND `default` = 1
                """,
                custom_company_id,
                as_dict=True
            )
    ...
    # FIX 6: Removed unused `random_id` variable that was generated but never used.
    terms = data.get("terms") or {}
    ...
    frappe.db.commit()
    send_response(status="success", message="Company updated", status_code=200, http_status=200)
...
def update_company_files():
    ...
    # FIX 1 (file saving): Now using save_file_to_frappe() instead of the
    # old save_file() which had a hardcoded site_name and wrong path building.
    if companyLogoFile:
        url = save_file_to_frappe(companyLogoFile)
        if not url:
            return send_response(status="fail", message="Failed to save company logo", status_code=500, http_status=500)
        company.company_logo = url

    if authorizedSignatureFile:
        url = save_file_to_frappe(authorizedSignatureFile)
        if not url:
            return send_response(status="fail", message="Failed to save authorized signature", status_code=500, http_status=500)
