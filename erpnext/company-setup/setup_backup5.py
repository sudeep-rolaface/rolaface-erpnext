...
#   2. os.getcwd() + site_name builds wrong path on server
#   3. URL returned as /files/uploads/logos/ — Nginx only serves /files/
# FIX:
#   1. frappe.get_site_path() auto-resolves correct path on any server
#   2. Files saved directly to public/files/ — no subfolders
#   3. URL returned as /files/filename — matches Nginx mapping exactly
# ============================================================
...
def get_company_api():
    try:
        ...
        # FIX: 'default' is a MySQL reserved word — must be wrapped in backticks
        bank_accounts_docs = frappe.get_all(
            ...
            fields=[
                ...
                "currency",
                "dateadded",
                "`default`",      # ✅ FIX: backticks escape MySQL reserved word
                "openingbalance"
            ]
        )
...
def create_company_api():
    ...
    for bank in accounts_map:
        ...
        default_count = sum(1 for bank in bank_accounts if int(bank.get("default", 0)) == 1)
...
def update_company_api():
    ...
    for account in bank_accounts:
        ...
        if is_default:
            # FIX: 'default' is a MySQL reserved word — must be wrapped in backticks in filters
            other_defaults = frappe.get_all(
                "Company Accounts",
                filters={"company_id": custom_company_id, "`default`": 1},  # ✅ FIX
                fields=["name"]
            )
