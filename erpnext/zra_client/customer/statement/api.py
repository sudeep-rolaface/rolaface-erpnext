...
def get_customer_statement():
    ...
    # ── Total Debit & Total Credit across ALL GL entries ──────────────────────
    total_debit  = sum(row.get("debit") or 0 for row in gl_rows)
    total_credit = sum(row.get("credit") or 0 for row in gl_rows)
    ...
    if page > 1:
        for row in gl_rows[:start]:
            running_balance += (row.get("debit") or 0) - (row.get("credit") or 0)
    ...
    statement = {
        ...
        "summary": {
            ...
            "totalDebit": total_debit,        # ✅ NEW
            "totalCredit": total_credit       # ✅ NEW
        },
        "aging": aging,
        "ledger": ledger,
        "pagination": {
