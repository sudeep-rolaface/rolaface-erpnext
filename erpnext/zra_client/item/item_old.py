...
def is_zra_sync_enabled():
    """
    Check if ZRA sync is enabled in settings.
    You can implement this based on your requirements:
    - System settings
    - Site config
    - Parameter passed in request
    - Environment variable
    """
    # Option 1: Check from a custom settings doctype
    try:
    ...
    # Option 2: Check from site_config.json
    return frappe.conf.get("enable_zra_sync", False)
    
    # Option 3: Default to False (ZRA sync disabled by default)
    # return False


def sync_item_with_zra(payload, is_update=False):
    """
    Helper function to sync item with ZRA.
...
def create_item_api():
    ...
    # Optional parameter to force ZRA sync for this specific request
    force_zra_sync = data.get("forceZraSync", False)
    
    # Check if ZRA sync will be performed
    ...
    # ZRA-specific required fields (only required when ZRA sync is enabled)
    if zra_will_sync:
        if not taxCategory:
            return send_response(
                status="error",
                message="taxCategory is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
        ...
        if not custom_orgnnatcd:
            return send_response(
                status="fail",
                message="Item origin code is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
        ...
        if not custom_pkgunitcd:
            return send_response(
                status="fail",
                message="Item packaging unit code is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
        ...
        if not custom_itemclscd:
            return send_response(
                status="fail",
                message="Item classification code is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
        ...
        if not custom_itemtycd:
            return send_response(
                status="fail",
                message="Item type code is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
        ...
        if not taxCode:
            return send_response(
                status="fail",
                message="taxCode is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
        ...
        if not custom_svcchargeyn:
            return send_response(
                status="fail",
                message="Service charge flag is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
        ...
        if not custom_isrcaplcbyn:
            return send_response(
                status="fail",
                message="Insurance applicable flag is required when ZRA sync is enabled",
                status_code=400,
                http_status=400
            )
    
    # Selling price is always required
    ...
    # Generate item code
    if zra_will_sync:
        # Use ZRA format when syncing
        item_code = generate_item_code_random(
            custom_orgnnatcd, 
            custom_itemtycd, 
            custom_pkgunitcd, 
            unit_of_measure_cd
        ...
        # Generate simple item code when not syncing with ZRA
    ...
    if zra_enabled:
        ...
        if not zra_success:
            # ZRA sync failed - you can choose to:
            # Option 1: Fail the entire operation (strict mode)
            # return send_response(
            #     status="error",
            #     message=zra_message,
            #     status_code=400,
            ...
            # Option 2: Continue with warning (lenient mode)
...
def update_item_api():
    ...
    # Validate taxCode and taxCategory only if ZRA sync is enabled
    if zra_will_sync:
        ...
        allowed_categories = ZRA_CLIENT_INSTANCE.getTaxCategory()
        if taxCategory and taxCategory not in ZRA_CLIENT_INSTANCE.getTaxCategory():
            return send_response(
                status="error",
                message=f"taxCategory must be one of {allowed_categories}",
                status_code=400,
