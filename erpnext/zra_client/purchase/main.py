...
class zraPurchase(ZRAClient):
    ...
    def create_purchase(self, purchase_data):
        ...
        # ✅ ZRA disabled — skip entirely, let ERPNext handle stock natively
        if not frappe.conf.get("enable_zra_sync", False):
            frappe.logger().info(
                "[ZRA PURCHASE] ZRA disabled — skipping ZRA purchase sync. "
                "ERPNext default flow will handle stock."
            )
            return

        # ── ZRA enabled — proceed with ZRA purchase sync ──────────────────────
        payload = {
        ...
        for idx, item in enumerate(purchase_data.get("items", [])):
            ...
            bins = frappe.db.get_all(
                ...
                fields=["actual_qty", "projected_qty", "name"]
            )
            ...
            try:
                ...
                frappe.log_error(f"Item not found: {item_code}", "ZRA Purchase")
                continue

            # ✅ Safe access — use getattr with fallback, no direct attribute access
            get_packaging_unit = (
                getattr(item_doc, "custom_packaging_unit_code", None) or
                getattr(item_doc, "custom_pkgunitcd", None) or
                "PCS"
            )

            try:
                r = requests.get(
                    f"http://0.0.0.0:7000/packaging-unit-code/{get_packaging_unit}/",
                    timeout=5
                )
                r.raise_for_status()
                packaging_unit_code = r.json().get("code") or get_packaging_unit
            except Exception as e:
                frappe.log_error(f"Failed to fetch packaging unit code for '{get_packaging_unit}': {e}", "ZRA Purchase")
                packaging_unit_code = get_packaging_unit  # ✅ fallback to raw value

            # ✅ Safe access — use getattr with fallback
            get_qty_unit = (
                getattr(item_doc, "custom_units_of_measure", None) or
                getattr(item_doc, "custom_itemunitcd", None) or
                item_doc.stock_uom or
                "PCS"
            )

            try:
                r = requests.get(
                    f"http://0.0.0.0:7000/unitofmeasure/{get_qty_unit}/",
                    timeout=5
                )
                r.raise_for_status()
                qty_unit_code = r.json().get("code") or get_qty_unit
            except Exception as e:
                frappe.log_error(f"Failed to fetch qty unit code for '{get_qty_unit}': {e}", "ZRA Purchase")
                qty_unit_code = get_qty_unit  # ✅ fallback to raw value

            # ✅ Safe access — use getattr with fallback
            item_cls_cd = (
                getattr(item_doc, "unspsc_code", None) or
                getattr(item_doc, "custom_item_class_code", None) or
                "50102517"
            )
        ...
        try:
            ...
            frappe.log_error(f"ZRA save_purchase_manually failed: {e}", "ZRA Purchase")
            return  # ✅ non-blocking — don't raise, let ERPNext continue
        ...
        if response_data.get("resultCd") == "000":
            ...
            for item in toUseData.get("itemList", []):
            ...
            # ✅ Non-blocking — log error but don't throw, let ERPNext continue
            frappe.log_error(
                f"ZRA Purchase save failed: {response_data.get('resultMsg')}",
                "ZRA Purchase"
            )
            frappe.logger().info(
                f"[ZRA PURCHASE] resultCd={response_data.get('resultCd')} "
                f"msg={response_data.get('resultMsg')}"
            )
