...
def get_company_currency():
    ...
    Uses direct SQL to bypass Frappe's document/redis cache so changes made in
    the Company form are reflected immediately without needing a cache clear.
    """
    # Strategy 1: user-level default company
    company = frappe.defaults.get_user_default("Company")

    # Strategy 2: global default company (bypass cache with direct SQL)
    if not company:
        company = frappe.db.sql(
            "SELECT defvalue FROM `tabDefaultValue` WHERE defkey = 'default_company' LIMIT 1"
        )
    ...
    # Direct SQL bypasses all caching layers — always returns the live DB value
...
def create_purchase_order():
    ...
    # Normalise: always uppercase, fall back to company currency if not provided
    if not currency:
    ...
    # If the caller explicitly passed a conversionRate, honour it (always takes priority)
    ...
    if explicit_rate:
        ...
        # Different currency — try to resolve automatically from Currency Exchange records
        ...
        if conversion_rate is None:
            return send_response(
                ...
                message=(
                    ...
                    f"  (1) Add \"conversionRate\": <rate> to your request payload (e.g. \"conversionRate\": 0.0095), OR\n"
                    f"  (2) Create a Currency Exchange record in ERPNext: "
                    f"Accounting → Currency Exchange → New → From {currency} To {company_currency}."
                ),
                data=[],
                status_code=400,
                http_status=400,
