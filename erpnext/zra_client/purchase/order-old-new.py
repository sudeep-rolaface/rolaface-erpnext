...
def create_purchase_order():
    ...
    #  When disabled, compute totals directly from the taxes[] payload    #
    # ------------------------------------------------------------------ #
    if is_zra_enabled():
        sale_payload = {
            "name": 1,
        ...
        # Compute tax totals from the taxes array sent in the payload
        ...
        # If taxableAmount not provided in taxes, derive from items
        if total_taxable_amount == 0:
            total_taxable_amount = sum(
                float(i.get("rate") or 0) * float(i.get("quantity") or 0)
                for i in items
            )
