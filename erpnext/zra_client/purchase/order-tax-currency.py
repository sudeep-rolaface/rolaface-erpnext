    ...
    if is_zra_enabled():
        ...
        if not taxes:
            ...
            if total_tax_amount > 0:
                # Must use a valid Expenses/Valuation account, not stock expense account
                valid_tax_accounts = CUSTOM_FRAPPE_INSTANCE.GetExpensesValuationAccount()
                tax_account_head = valid_tax_accounts[0] if valid_tax_accounts else CUSTOM_FRAPPE_INSTANCE.getDefaultExpenseAccount()
                taxes = [{
                    "type": "Actual",
