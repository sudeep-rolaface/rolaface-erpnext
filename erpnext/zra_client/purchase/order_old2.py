...
def get_conversion_rate(from_currency, to_currency="ZMW", transaction_date=None):
    """
    Fetch the exchange rate between two currencies.
    Priority:
      1. Currency Exchange doctype (exact date match)
      2. Currency Exchange doctype (latest record)
      3. Frappe's built-in get_exchange_rate utility
      4. Default to 1.0 (same currency)
    """
...
def create_purchase_order():
    ...
    company_currency = (
        frappe.db.get_value("Company", company, "default_currency") or "ZMW"
    )
        ...
        if conversion_rate is None:
            return send_response(
                ...
                message=(
                    ...
                    f"Please create a Currency Exchange record in ERPNext for this pair "
                    f"(Accounting → Currency Exchange) and try again."
                ),
                data=[],
                status_code=400,
                http_status=400,
