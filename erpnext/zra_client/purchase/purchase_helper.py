...
class PurchaseHelper(ZRAClient):
    ...
    def send_purchase_data(self, purchase_data):
        ...
        # Check if ZRA is enabled - do this AFTER building payload
        if not self.is_zra_enabled():
            print("[INFO] ZRA sync is disabled in site_config.json. Purchase data processed locally only.")
            return {
                "resultCd": "000",
                "resultMsg": "Success - ZRA integration disabled in configuration",
                "payload": payload,  # Return the payload for local processing
                "zra_enabled": False
            }
        
        # Only call ZRA API if enabled
