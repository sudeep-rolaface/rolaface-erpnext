...
class DebitNoteSale(ZRAClient):
    ...
    def build_payload(self, items, base_data):
        ...
        originInvoice = base_data.get("originInvoice")
        if originInvoice:
            if hasattr(originInvoice, "name"):
                originInvoice = originInvoice.name
            if originInvoice.startswith("Sales Invoice (") and originInvoice.endswith(")"):
                orgInvcNo = originInvoice[len("Sales Invoice ("):-1].strip()

            originRcptNo = self.get_sales_rcptno_by_inv_no_c(originInvoice)
        else:
            originRcptNo = None
        ...
        if not originRcptNo:
            send_response(
                status="fail",
                message=f"No receipt number found for Sales Invoice '{originInvoice}'",
                status_code=404,
                http_status=404
            )
            return
        ...
        payload = {
            ...
            **self.generate_tax_fields(),
            ...
            "modrNm": username,
            "saleCtyCd": "1",
            "dbtRsnCd": base_data["DebitNoteReasonCode"],
    ...
    def send_sale_data(self, sell_data):
        ...
        originInvoice = sell_data.get("originInvoice")
        DebitNoteReasonCode = sell_data.get("DebitNoteReasonCode")
